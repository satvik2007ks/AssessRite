...
namespace AssessRite
{
    public partial class ManualResultEntry : System.Web.UI.Page
    {
        ...
        {
            string qur = "Select TestId,TestKey from Test where CreatedBy='" + Session["UserId"].ToString() + "' and TestType='Offline'";
        ...
        private void Assign(bool found)
        {
            ...
            {
                string qur = dbLibrary.idBuildQuery("[proc_ScheduleTest]", Session["TestId"].ToString(), Session["TestKey"].ToString(), "", "", "", ddlClass.SelectedValue.ToString());
                TestScheduleId = dbLibrary.idGetAFieldByQuery(qur);
                if (!string.IsNullOrEmpty(TestScheduleId))
                {
                    qur = dbLibrary.idBuildQuery("[proc_AssignOfflineTest]", TestScheduleId, ddlStudent.SelectedValue);
